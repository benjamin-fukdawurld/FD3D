#ifndef FD3D_LIGHT_JSON_H
#define FD3D_LIGHT_JSON_H
...
{
    rapidjson::Value result = serializer.serialize(static_cast<const FD3D::Component &>(light));
    ...
    result.AddMember(serializer.serialize("attenuation"),
                     ...
                     serializer.getAllocator());
    result.AddMember(serializer.serialize("lightType"),
...
{
    if(!serializer.unserialize(val, static_cast<FD3D::Component&>(light), err))
        return false;
...

    {
        ...
        FD3D::LightType t;
        const rapidjson::Value &type = val["lightType"];
